...
import { Brain, Cpu, Trophy, ArrowRight, RotateCcw, CheckCircle2, XCircle, ChevronLeft, Swords, BookOpen, Lightbulb, Film } from 'lucide-react';

// ==========================================
// 📚 RAW QUIZ DATA (COMPRESSED FORMAT)
...
const rawQuizData = {
  ...
  history: {
    ...
    intermediate: [
      ["What was the name of the document signed by King John of England in 1215 that limited his powers?", "Magna Carta", "The Bill of Rights", "The Declaration of Rights", "The Domesday Book"],
    ...
    advanced: [
      ["Which treaty concluded the Thirty Years' War in 1648 and established the concept of modern state sovereignty?", "The Peace of Westphalia", "The Treaty of Utrecht", "The Congress of Vienna", "The Treaty of Tordesillas"],
  ...
  funfact: {
    foundational: [
      ["What is the fastest land animal in the world?", "Cheetah", "Lion", "Horse", "Greyhound"],
    ...
    advanced: [
      ["What is the name of the protein that makes fireflies glow?", "Luciferin", "Biolumin", "Phosphorin", "Luminol"],
  ...
  entertainment: {
    foundational: [
      ["Who is the famous mouse created by Walt Disney?", "Mickey Mouse", "Mighty Mouse", "Jerry", "Stuart Little"],
    ...
    intermediate: [
      ["Which director is known for films like 'Inception', 'Titanic', and 'The Terminator'?", "James Cameron", "Steven Spielberg", "Christopher Nolan", "Quentin Tarantino"],
    ...
    advanced: [
      ["Which film won the first-ever Academy Award for Best Animated Feature in 2002?", "Shrek", "Monsters, Inc.", "Jimmy Neutron: Boy Genius", "Spirited Away"],
...
// ==========================================
// PARSE COMPRESSED DATA INTO OBJECTS
// ==========================================
...
// ==========================================
const SUBJECTS = [
  { id: 'science', title: 'Science', icon: Brain, theme: 'standard', description: 'Explore the natural world and universe.' },
  { id: 'tech', title: 'Tech & Math', icon: Cpu, theme: 'standard', description: 'Test your digital logic and mathematics.' },
  { id: 'history', title: 'History', icon: BookOpen, theme: 'standard', description: 'Journey through the annals of human civilization.' },
  { id: 'funfact', title: 'Fun Facts', icon: Lightbulb, theme: 'standard', description: 'Fascinating and weird trivia from around the world.' },
  { id: 'entertainment', title: 'Entertainment', icon: Film, theme: 'standard', description: 'Movies, music, games, and pop culture.' },
  { id: 'lore', title: 'Nichothéos', icon: Swords, theme: 'celestial', description: 'Delve into the mythos of Aetherius Dominus & Jayden.' }
];

const DIFFICULTIES = [
  { id: 'foundational', title: 'Foundational', color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30 hover:border-emerald-400' },
  { id: 'intermediate', title: 'Intermediate', color: 'bg-blue-500/10 text-blue-400 border-blue-500/30 hover:border-blue-400' },
  { id: 'advanced', title: 'Advanced', color: 'bg-rose-500/10 text-rose-400 border-rose-500/30 hover:border-rose-400' }
];
...
export default function App() {
  ...
  const startQuiz = (difficultyId) => {
    ...
    let qList = quizData[selectedSubject.id][difficultyId];
    ...
    const randomizedQuestions = shuffleArray(qList).map(q => ({
      ...q,
      options: shuffleArray(q.options)
    }));

    setQuestions(randomizedQuestions.slice(0, 10)); 
    setCurrentIndex(0);
    setScore(0);
  ...
  const isCelestial = selectedSubject?.theme === 'celestial';
    ...
    <div className="animate-in fade-in zoom-in duration-500 max-w-5xl mx-auto w-full">
      <div className="text-center mb-12">
        ...
        <p className="text-slate-400 text-lg max-w-2xl mx-auto">
          Choose a subject to test your knowledge. Over 700 questions await!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {SUBJECTS.map((sub) => {
          const Icon = sub.icon;
          const isGold = sub.theme === 'celestial';
          return (
            <button
              key={sub.id}
              onClick={() => { setSelectedSubject(sub); setGameState('difficulty_select'); }}
              className={`relative flex flex-col items-center p-8 rounded-2xl border transition-all duration-300 group text-left
                ${isGold 
                  ? 'bg-slate-900/50 border-amber-500/30 hover:border-amber-400 hover:shadow-[0_0_30px_rgba(251,191,36,0.2)]' 
                  : 'bg-slate-900/50 border-slate-700 hover:border-blue-400 hover:shadow-[0_0_30px_rgba(96,165,250,0.15)]'}`}
            >
              <div className={`mb-6 p-4 rounded-full transition-transform duration-300 group-hover:scale-110 
                ${isGold ? 'bg-amber-500/10 text-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.4)]' : 'bg-blue-500/10 text-blue-400'}`}>
                <Icon size={32} />
              </div>
              <h3 className={`text-xl font-bold mb-3 ${isGold ? 'text-amber-50' : 'text-white'}`}>
                {sub.title}
              </h3>
  ...
  const renderDifficultySelect = () => (
    <div className="animate-in slide-in-from-right-8 duration-500 max-w-2xl mx-auto w-full">
      <button 
      ...
      <div className="text-center mb-10">
        <div className={`inline-flex p-4 rounded-full mb-4 ${isCelestial ? 'bg-amber-500/10 text-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.4)]' : 'bg-blue-500/10 text-blue-400'}`}>
           {selectedSubject && React.createElement(selectedSubject.icon, { size: 40 })}
        </div>
        <h2 className={`text-3xl font-bold mb-2 ${isCelestial ? 'text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-amber-500' : 'text-white'}`}>
          {selectedSubject.title}
        </h2>
        <p className="text-slate-400">Select your proficiency level to begin</p>
      </div>
      ...
      <div className="flex flex-col space-y-4">
        {DIFFICULTIES.map((diff) => (
          <button
            ...
            className={`flex items-center justify-between p-6 rounded-xl border bg-slate-900/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${diff.color}`}
          >
            <span className="text-xl font-semibold tracking-wide">{diff.title}</span>
      ...
      <div className="animate-in fade-in duration-500 max-w-3xl mx-auto w-full">
        <div className="flex items-center justify-between mb-8 text-sm font-medium text-slate-400">
          <div className="flex items-center space-x-2">
            <span className={`px-3 py-1 rounded-full border ${isCelestial ? 'border-amber-500/30 text-amber-400 bg-amber-500/10' : 'border-slate-700 bg-slate-800'}`}>
              {selectedSubject.title}
            </span>
            <span className="px-3 py-1 rounded-full border border-slate-700 bg-slate-800 capitalize">
              {selectedDifficulty}
            </span>
          </div>
          <div className="flex items-center space-x-4">
            <span>Score: <span className={isCelestial ? 'text-amber-400 font-bold' : 'text-white font-bold'}>{score}</span></span>
        ...
        <div className="w-full h-2 bg-slate-800 rounded-full mb-10 overflow-hidden">
          <div 
            className={`h-full rounded-full transition-all duration-500 ease-out ${isCelestial ? 'bg-gradient-to-r from-amber-500 to-yellow-300' : 'bg-gradient-to-r from-blue-500 to-purple-500'}`}
            style={{ width: `${progress}%` }}
          />
        </div>

        <div className={`p-8 rounded-2xl border bg-slate-900/80 backdrop-blur-sm shadow-2xl mb-8
          ${isCelestial ? 'border-amber-500/20 shadow-amber-900/20' : 'border-slate-800'}`}>
          <h2 className={`text-2xl md:text-3xl font-semibold mb-8 leading-tight ${isCelestial ? 'text-amber-50' : 'text-white'}`}>
            {currentQ.text}
          </h2>

          <div className="grid grid-cols-1 gap-4">
            {currentQ.options.map((option, idx) => {
              let btnClass = `flex items-center justify-between p-5 rounded-xl border text-left transition-all duration-200 font-medium text-lg`;
              let icon = null;
              ...
              if (isChecking) {
                ...
                btnClass += ` bg-slate-800/50 border-slate-700 text-slate-200 hover:bg-slate-800 hover:border-slate-500 cursor-pointer`;
                if (isCelestial) {
                   btnClass += ` hover:border-amber-500/50 hover:bg-amber-900/10`;
                }
              }
  ...
  const renderResults = () => {
    ...
    if (percentage === 100) message = "Perfect Score! Masterful.";
    else if (percentage >= 80) message = "Excellent work!";
    else if (percentage >= 50) message = "Well done, but there's room to grow.";

    return (
      <div className="animate-in zoom-in duration-500 max-w-xl mx-auto w-full text-center">
        <div className={`p-10 rounded-3xl border bg-slate-900/80 backdrop-blur-sm shadow-2xl relative overflow-hidden
          ${isCelestial ? 'border-amber-500/30 shadow-[0_0_50px_rgba(251,191,36,0.15)]' : 'border-slate-800'}`}>
          
          <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 rounded-full blur-3xl opacity-20 pointer-events-none
            ${isCelestial ? 'bg-amber-500' : 'bg-blue-500'}`} />

          <div className="relative z-10">
            <div className={`inline-flex p-5 rounded-full mb-6 ${isCelestial ? 'bg-amber-500/20 text-amber-400' : 'bg-blue-500/20 text-blue-400'}`}>
              <Trophy size={48} />
            </div>
            
            <h2 className="text-4xl font-bold text-white mb-2">Quiz Complete</h2>
            <p className={`text-xl font-medium mb-8 ${isCelestial ? 'text-amber-200/80' : 'text-blue-200/80'}`}>
              {message}
            </p>

            <div className="flex justify-center items-center space-x-8 mb-10">
              <div className="text-center">
                <p className="text-slate-400 text-sm uppercase tracking-wider mb-1">Final Score</p>
                <p className={`text-5xl font-black ${isCelestial ? 'text-amber-400' : 'text-white'}`}>
                  {score}<span className="text-2xl text-slate-500">/{questions.length}</span>
                </p>
              </div>
              <div className="w-px h-16 bg-slate-800"></div>
              <div className="text-center">
                <p className="text-slate-400 text-sm uppercase tracking-wider mb-1">Accuracy</p>
                <p className={`text-5xl font-black ${isCelestial ? 'text-amber-400' : 'text-white'}`}>
                  {percentage}<span className="text-2xl text-slate-500">%</span>
                </p>
              </div>
            </div>
            ...
            <button
              onClick={resetQuiz}
              className={`flex items-center justify-center w-full py-4 rounded-xl font-bold text-lg transition-all duration-300
                ${isCelestial 
                  ? 'bg-gradient-to-r from-amber-600 to-yellow-500 hover:from-amber-500 hover:to-yellow-400 text-amber-950 shadow-[0_0_20px_rgba(251,191,36,0.3)]' 
                  : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white shadow-[0_0_20px_rgba(59,130,246,0.3)]'}`}
            >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-blue-500/30 flex flex-col">
      {isCelestial && (
        <div className="fixed inset-0 pointer-events-none bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-amber-900/10 via-slate-950 to-slate-950 transition-opacity duration-1000 z-0" />
      )}
      
      <header className="relative z-10 p-6 flex justify-center border-b border-slate-800/50 bg-slate-950/50 backdrop-blur-md">
        <div className="flex items-center space-x-2 cursor-pointer" onClick={resetQuiz}>
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-2 rounded-lg">
            <Brain className="text-white" size={24} />
          </div>
          <span className="text-2xl font-black tracking-tight text-white">
            Nexus<span className="text-slate-500">Quiz</span>
          </span>
        </div>
      </header>

      <main className="relative z-10 flex-1 flex items-center justify-center p-6 md:p-12">
        {gameState === 'subject_select' && renderSubjectSelect()}
        {gameState === 'difficulty_select' && renderDifficultySelect()}
        {gameState === 'playing' && renderActiveQuiz()}
